 ...
 * Always (de)serialises via helpers from annotationTypes.ts.
 */
...
function unwrapStrapi<T>(data: any): T {
  ...
  /**
   * Fetch annotations for a given literature + PDF (hash).
   * If no filter is supplied, returns *all* annotations.
   */
  export async function fetchAnnotations(
    ...
    if (!res.ok) {
      throw new Error(`Fetch annotations failed: ${res.status}`);
    }
