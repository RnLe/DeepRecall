...
import { previewImport, executeImport, formatBytes } from "@/src/utils/data-sync";
import type { ImportPreview, ImportOptions, ImportStrategy } from "@/src/schema/data-sync";
...
export function ImportDataDialog({ isOpen, onClose, onSuccess }: ImportDataDialogProps) {
  ...
  const totalConflicts = preview ? Object.values(preview.conflicts).reduce((sum, n) => sum + n, 0) : 0;
              ...
              <div className="space-y-2">
                ...
                <div className="p-4 bg-zinc-50 dark:bg-zinc-800/50 rounded-lg space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-zinc-600 dark:text-zinc-400">Exported:</span>
                    <span>{new Date(preview.metadata.exportedAt).toLocaleString()}</span>
                  </div>
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div className="space-y-1">
                      {preview.warnings.map((warning, i) => (
                        <p key={i} className="text-sm text-yellow-900 dark:text-yellow-100">
                          {warning}
                        </p>
                      ))}
                    </div>
                ...
                <div className="space-y-2">
                  <label className="flex items-start gap-3 p-4 border rounded-lg cursor-pointer hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-zinc-600 dark:text-zinc-400">
                        Add new items and update existing ones. Keeps your current data.
                      </p>
                      {totalConflicts > 0 && (
                  ...
                  <label className="flex items-start gap-3 p-4 border rounded-lg cursor-pointer hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-zinc-600 dark:text-zinc-400">
                        Remove all existing data and import fresh. Cannot be undone.
                      </p>
                ...
                <div className="space-y-2">
                  <label className="flex items-center gap-3 p-3 bg-zinc-50 dark:bg-zinc-800/50 rounded-lg">
                    ...
                    <div className="flex-1">
                      <p className="text-sm font-medium">Knowledge Data (Required)</p>
                      <p className="text-xs text-zinc-500">
                        {preview.metadata.counts.works} works, {preview.metadata.counts.assets} assets, etc.
                      </p>
                    </div>
                  </label>
                  ...
                  <label className="flex items-center gap-3 p-3 bg-zinc-50 dark:bg-zinc-800/50 rounded-lg cursor-pointer">
                    <input
                      ...
                      onChange={(e) => setOptions({ ...options, importSQLite: e.target.checked })}
                      className="mt-0"
                    />
                    <div className="flex-1">
                      <p className="text-sm font-medium">File Metadata</p>
                      <p className="text-xs text-zinc-500">
                        {preview.metadata.counts.blobs} blobs, {preview.metadata.counts.paths} paths
                      </p>
                    </div>
                  </label>
                  ...
                  <label className="flex items-center gap-3 p-3 bg-zinc-50 dark:bg-zinc-800/50 rounded-lg cursor-pointer">
                    <input
                      ...
                      onChange={(e) => setOptions({ ...options, importFiles: e.target.checked })}
                      className="mt-0"
                    />
                    <div className="flex-1">
                      <p className="text-sm font-medium">Other Files</p>
                      <p className="text-xs text-zinc-500">
                        {preview.metadata.counts.files} files (avatars, db files, etc.)
                      </p>
                    </div>
                  </label>
                </div>
