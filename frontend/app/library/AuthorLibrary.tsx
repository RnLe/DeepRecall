...
import { formatAuthorsForDisplay } from "@/src/utils/authorDisplay";
...
type View = "list" | "edit" | "create" | "import";
...
function AuthorListView({
  ...
  const authorWorkCounts = useMemo(() => {
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4">
        {authors.length === 0 ? (
          ...
          <div className="space-y-2">
            {authors.map((author) => {
              const workCount = authorWorkCounts.get(author.id) || 0;
              return (
                <button
                  key={author.id}
                  onClick={() => onSelectAuthor(author.id)}
                  className="w-full p-4 bg-neutral-800/50 hover:bg-neutral-800 border border-neutral-700 hover:border-neutral-600 rounded-lg transition-all text-left group"
                >
                  <div className="flex items-start justify-between">
                    ...
                    <Edit2 className="w-4 h-4 text-neutral-500 group-hover:text-neutral-300 transition-colors flex-shrink-0" />
                  </div>
                </button>
              );
            })}
          ...
          {authorWorks.length > 0 && (
            <div className="p-4 bg-blue-950/20 border border-blue-900/30 rounded-lg">
              <h3 className="text-sm font-semibold text-blue-300 mb-2 flex items-center gap-2">
                ...
                Connected Works ({authorWorks.length})
              </h3>
              <div className="space-y-1">
                {authorWorks.slice(0, 5).map((work) => (
                  <div
                    key={work.id}
                    className="text-sm text-blue-200/80 truncate"
                  >
                    • {work.title}
                  </div>
                ))}
