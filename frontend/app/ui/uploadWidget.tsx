...
import { LITERATURE_TYPES, LiteratureType, mapLiteratureItems } from "../helpers/literatureTypes";
...
const UploadWidget: React.FC<UploadWidgetProps> = ({ className }) => {
  // State for the selected literature type
  ...
  // Reference to the hidden file input (if needed)
  ...
  const entries = useMemo(() => {
    ...
    // Use the helper to map entries for the selected type.
    return mapLiteratureItems(data, selectedType as LiteratureType);
  }, [data, selectedType]);
      ...
      {selectedType && (
        <div className="flex gap-4 mt-4">
          ...
          <div className="w-1/2 pl-2">
            {formsVisible &&
              (selectedEntry ? (
                <VersionForm mediaType={selectedType as LiteratureType} entry={selectedEntry} />
              ) : (
                <LiteratureForm mediaType={selectedType as LiteratureType} onSuccess={handleFormInvisibility} />
              ))}
          </div>
        </div>
      )}
