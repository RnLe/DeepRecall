...
const LiteratureCardM: React.FC<LiteratureCardProps> = ({ literature, showThumbnail = true, onClick, onPdfPreview }) => {
  ...
  const hasAdditionalInfo = subtitle || authors.length > 0 || journal || doi || publisher || Object.keys(safeCustomMetadata).length > 0;
          ...
          <div className="flex items-start justify-between mb-4">
            ...
            {hasAdditionalInfo && (
              <button
                ...
                className="ml-3 p-2 rounded-lg bg-slate-700/50 text-slate-400 hover:text-slate-200 hover:bg-slate-600/50 transition-colors opacity-0 group-hover:opacity-100"
              >
                <svg
          ...
          {authors.length > 0 && (
            <div className="flex items-center space-x-2 mb-3">
              ...
              <div className="flex flex-wrap gap-1">
                {authors.slice(0, isExpanded ? authors.length : 3).map((author, index) => (
                  <span
                    key={index}
                ...
                {!isExpanded && authors.length > 3 && (
                  <span className="text-sm text-slate-400">+{authors.length - 3} more</span>
                )}
              </div>
            </div>
          )}
