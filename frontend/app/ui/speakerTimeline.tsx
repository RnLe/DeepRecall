...
export const SpeakerTimeline: React.FC<SpeakerTimelineProps> = ({ results, totalDuration, assignedMapping, availableSpeakers, audioSrc }) => {
  ...
  const speakers = Array.from(new Set(results.map(r => r.speaker)));
  const canvasWidth = containerWidth;
  const canvasHeight = speakers.length * barHeight + barHeight; // Add one extra row for the invisible bar
  ...
  useEffect(() => {
    ...
    results.forEach(result => {
      const speakerIndex = speakers.indexOf(result.speaker);
      ...
      if (assignedId) {
        ...
        const hue = (speakerIndex * 137.508) % 360;
        fillColor = `hsl(${hue}, 70%, 50%)`;
      }
      ctx.fillStyle = fillColor;
      ctx.fillRect(x, y, width, barHeight - 2);
    });
  }, [results, totalDuration, speakers, assignedMapping, availableSpeakers, canvasWidth, canvasHeight]);
