...
function deserializeExampleManyToManyCollection(
  collection: ExampleManyToManyCollectionStrapi
): ExampleManyToManyCollection {
  return {
    ...collection,
  } as ExampleManyToManyCollection;
}

function serializeExampleManyToManyCollection(
  collection: ExampleManyToManyCollection
): ExampleManyToManyCollectionStrapi {
  return {
    ...collection,
  } as ExampleManyToManyCollectionStrapi;
}

function deserializeExampleOneToOneCollection(
  collection: ExampleOneToOneCollectionStrapi
): ExampleOneToOneCollection {
  return {
    ...collection,
  } as ExampleOneToOneCollection;
}

function serializeExampleOneToOneCollection(
  collection: ExampleOneToOneCollection
): ExampleOneToOneCollectionStrapi {
  return {
    ...collection,
...
function deserializeExample(card: ExampleStrapi): Example {
  ...
  return {
    ...
    oneToOne: oneToOne
      ? deserializeExampleOneToOneCollection(oneToOne)
      : undefined,
  };
}
...
function serializeExample(card: Example): ExampleStrapi {
  ...
  const manyToManyIds =
    card.manyToMany?.map((collection) => collection.documentId) || [];
