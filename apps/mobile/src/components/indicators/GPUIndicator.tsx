...
export function GPUIndicator() {
  ...
  if (status.available) {
    tooltipLines.push(`✅ GPU Available (${status.backend.toUpperCase()})`);
    if (status.renderer) tooltipLines.push(`Renderer: ${status.renderer}`);
    if (status.vendor) tooltipLines.push(`Vendor: ${status.vendor}`);
  } else {
    tooltipLines.push("❌ No GPU Acceleration");
  }
  ...
  return (
    ...
    >
      ...
      <span
        className={`w-2 h-2 rounded-full ${dotColor} ${status.available ? "animate-pulse" : ""}`}
      />
    </div>
  );
}
