/**
 * PDFViewer - Mobile implementation
 * Full-featured PDF viewer with virtualization and annotations
 * Adapted from web version for React Router and Capacitor
 */
...
export function PDFViewer({ source, sha256, className = "" }: PDFViewerProps) {
  ...
  const { data: allAnnotations = [] } = usePDFAnnotations(sha256);
  ...
  return (
    <div className={`pdf-viewer flex flex-col h-full ${className}`}>
      {/* Toolbar - Mobile optimized with compact layout */}
      <div className="pdf-toolbar flex items-center gap-2 p-2 bg-gray-800 border-b border-gray-700">
        {/* Left: Zoom & Fit */}
        <div className="flex items-center gap-1">
          <button
            onClick={viewport.zoomOut}
            disabled={viewport.scale <= 0.5}
            className="px-2 py-1 bg-gray-700 border border-gray-600 rounded hover:bg-gray-600 disabled:opacity-40 disabled:cursor-not-allowed text-gray-200 text-xs font-medium transition-colors"
            title="Zoom out"
          >
            −
          </button>

          <span className="text-xs w-10 text-center text-gray-300 font-medium">
          ...
          <button
            onClick={fitToWidth}
            className="p-1 bg-gray-700 border border-gray-600 rounded hover:bg-gray-600 text-gray-200 transition-colors"
            title="Fit to width"
          >
          ...
          {Math.round(viewport.scale * 100) !== 100 && (
            <button
              ...
              className="p-1 bg-purple-600 border border-purple-500 rounded hover:bg-purple-700 text-white transition-colors"
        ...
        <div className="flex-1 flex items-center justify-center gap-1">
          <button
            ...
            className="p-1 bg-gray-700 border border-gray-600 rounded hover:bg-gray-600 disabled:opacity-40 text-gray-200"
            title="First page"
          >
          ...
          <div className="flex items-center gap-1 text-gray-300 text-xs">
            <input
              ...
              className="w-12 px-1 py-0.5 bg-gray-900 border border-gray-700 rounded text-gray-200 text-center text-xs"
              inputMode="numeric"
            />
      ...
      <div className="flex-1 relative">
        <div
          ...
          onClick={(e) => {
            // Close sidebar when clicking container background
            if (
