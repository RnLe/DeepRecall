...
import { usePresetUsageCount } from "@/src/hooks/useLibrary";

interface TemplateEditorModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (updates: Partial<Preset>) => Promise<void>;
  preset: Preset;
}
...
export function TemplateEditorModal({
  ...
  const usageCount = usePresetUsageCount(preset.id);
    ...
    <div className="fixed inset-0 z-80 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="bg-neutral-900 rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col border border-neutral-800">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {/* Basic Info Section */}
          <div
            className={`border border-neutral-800 rounded-lg overflow-hidden ${hasLinkedEntities ? "opacity-50 pointer-events-none" : ""}`}
          >
            <button
