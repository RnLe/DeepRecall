  ...
  const handleFileUpload = async (files: File[]) => {
    ...
    try {
      for (const file of files) {
        ...
        // Create Asset in Dexie with noteGroup
...
function NoteCard({
  ...
  useEffect(() => {
    if (viewMode === "detailed" && isMarkdown) {
      fetch(`/api/blob/${note.sha256}`)
        .then((res) => res.text())
        .then((text) => setMarkdownContent(text))
        .catch((err) => console.error("Failed to load markdown:", err));
    }
  }, [viewMode, isMarkdown, note.sha256]);
  ...
  if (viewMode === "compact") {
    ...
    const thumbnailUrl = showThumbnail ? `/api/blob/${note.sha256}` : null;
          ...
          {thumbnailUrl ? (
            <div className="shrink-0 w-16 h-16 bg-gray-900 rounded overflow-hidden">
              {isImage ? (
                ...
                <iframe
                  src={`/api/blob/${note.sha256}#page=1&view=FitH`}
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          ...
          {isPDF && (
            ...
            >
              <iframe
                src={`/api/blob/${note.sha256}#page=1&view=Fit`}
                className="w-full h-full pointer-events-none"
