...
export class Camera {
  ...
  zoomAt(screenPoint: Point, zoomFactor: number): void {
    const newScale = this.clampZoom(this.state.scale * zoomFactor);

    // Calculate the point in canvas coordinates (before transform)
    const canvasX = (screenPoint.x - this.state.panOffset.x) / this.state.scale;
    const canvasY = (screenPoint.y - this.state.panOffset.y) / this.state.scale;

    // Calculate new pan to keep the same canvas point under the mouse
    this.state.panOffset.x = screenPoint.x - canvasX * newScale;
    this.state.panOffset.y = screenPoint.y - canvasY * newScale;
