...
/**
 * Clean up synced presets from local table
 ...
 *
 * @param syncedPresets - Current synced presets from Electric
 */
export async function cleanupSyncedPresets(
  syncedPresets: Preset[]
): Promise<void> {
  ...
  return new Promise((resolve) => {
    cleanupTimer = setTimeout(async () => {
      await _cleanupSyncedPresetsInternal(syncedPresets);
      resolve();
    }, 100);
  });
}
 ...
 */
async function _cleanupSyncedPresetsInternal(
  syncedPresets: Preset[]
): Promise<void> {
  const localChanges = await db.presets_local.toArray();
  ...
  const currentHash = `${syncedPresets.length}-${localChanges.length}`;
  if (currentHash === lastCleanupHash) {
    return;
  }
  lastCleanupHash = currentHash;

  const syncedIds = new Set(syncedPresets.map((p) => p.id));
  let cleanedCount = 0;
  ...
  for (const local of localChanges) {
    ...
    switch (local._op) {
      case "insert":
        ...
        if (isSynced) {
          await db.presets_local.where("id").equals(local.id).delete();
          cleanedCount++;
          console.log(`[Cleanup] Confirmed insert for preset ${local.id}`);
        }
        break;
      ...
      case "update":
        ...
        if (isSynced) {
          const synced = syncedPresets.find((p) => p.id === local.id);
          ...
          if (synced && synced.updatedAt && local.data?.updatedAt) {
            ...
            if (syncedTime >= localTime) {
              await db.presets_local.where("id").equals(local.id).delete();
              cleanedCount++;
              console.log(`[Cleanup] Confirmed update for preset ${local.id}`);
            }
          }
        }
        break;
      ...
      case "delete":
        ...
        if (!isSynced) {
          await db.presets_local.where("id").equals(local.id).delete();
          cleanedCount++;
          console.log(`[Cleanup] Confirmed delete for preset ${local.id}`);
        }
        break;
    }
  }
...
export async function cleanupOldErrors(): Promise<void> {
  ...
  const oldErrors = await db.presets_local
    .where("_status")
    .equals("error")
...
export async function forceCleanupAll(): Promise<void> {
  const count = await db.presets_local
    .where("_status")
    .equals("synced")
    .delete();
