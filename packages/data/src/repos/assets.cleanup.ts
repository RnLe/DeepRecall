...
/**
 * Clean up synced assets from local table
 ...
 *
 * @param syncedAssets - Current synced assets from Electric
 */
export async function cleanupSyncedAssets(
  syncedAssets: Asset[]
): Promise<void> {
  ...
  return new Promise((resolve) => {
    cleanupTimer = setTimeout(async () => {
      await _cleanupSyncedAssetsInternal(syncedAssets);
      resolve();
    }, 100);
  });
}
 ...
 */
async function _cleanupSyncedAssetsInternal(
  syncedAssets: Asset[]
): Promise<void> {
  const localChanges = await db.assets_local.toArray();
  ...
  const currentHash = `${syncedAssets.length}-${localChanges.length}`;
  ...
  for (const local of localChanges) {
    ...
    switch (local._op) {
      ...
      case "update":
        ...
        if (isSynced) {
          const synced = syncedAssets.find((a) => a.id === local.id);
...
export async function cleanupOldErrors(): Promise<void> {
  ...
  const oldErrors = await db.assets_local
    .where("_status")
    .equals("error")
...
export async function forceCleanupAll(): Promise<void> {
  const count = await db.assets_local
    .where("_status")
    .equals("synced")
    .delete();
