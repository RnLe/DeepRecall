 ...
 *
 * Platform-agnostic component using @deeprecall/data
 */
...
export interface NoteSidebarOperations extends NotePreviewOperations {
  /** Fetch assets attached to an annotation */
...
export interface NoteSidebarProps {
  ...
  operations: NoteSidebarOperations;
}
        ...
        await Promise.all(
          pageAnnotations.map(async (ann) => {
            if (
              ...
              try {
                const notes = await operations.getAnnotationAssets(ann.id);
                if (notes.length > 0) {
              ...
              <div className="space-y-2">
                {notes.map((note) => (
                  <div key={note.id} className="text-xs">
                    <NotePreview asset={note} operations={operations} />
                  </div>
                ))}
              </div>
            </div>
