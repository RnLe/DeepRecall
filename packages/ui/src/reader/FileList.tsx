 ...
 * Displays Assets from Electric SQL with optimistic updates
 *
 * Platform-agnostic component using Electric SQL hooks
 */
...
export function FileList() {
  ...
  // Load data from Electric (merged: synced + local optimistic changes)
  const { data: assets, isLoading: assetsLoading } = useAssets();
  const { data: works, isLoading: worksLoading } = useWorks();
  const { data: presets, isLoading: presetsLoading } = usePresets();

  const isLoading = assetsLoading || worksLoading || presetsLoading;
...
function FileListItem({
  ...
  // Load annotations for this PDF (merged: synced + local)
  const { data: annotations } = usePDFAnnotations(asset.sha256);
