 ...
 *
 * Platform-agnostic component using @deeprecall/data hooks
 */

"use client";

import { useEffect, useState, useRef } from "react";
import { useAnnotationUI, useReaderUI } from "@deeprecall/data";
import {
  useAnnotation,
  useUpdateAnnotation,
  useDeleteAnnotation,
} from "@deeprecall/data/hooks";
import { useAssets, useDeleteAsset } from "@deeprecall/data/hooks";
...
} from "./CreateNoteDialog";
import {
  CompactNoteItem,
  type CompactNoteItemOperations,
} from "./CompactNoteItem";
import {
  NoteDetailModal,
...
export interface AnnotationEditorOperations
  extends CompactNoteItemOperations,
...
export interface AnnotationEditorProps {
  ...
  operations: AnnotationEditorOperations;
}
...
export function AnnotationEditor({
  ...
  // Fetch annotation using optimistic hook
  const { data: annotation, isLoading } = useAnnotation(
  ...
  // Fetch all assets (for filtering attached notes)
  ...
  // Compute attached notes from annotation metadata and all assets
  const attachedNotes = annotation?.metadata?.attachedAssets
    ? allAssets.filter((asset) =>
        annotation.metadata.attachedAssets?.includes(asset.id)
      )
    : [];

  // Load annotation data into form when annotation changes
  useEffect(() => {
    if (annotation) {
      setTitle(annotation.metadata?.title || "");
      setKind(annotation.metadata?.kind || "");
      setNotes(annotation.metadata?.notes || "");
      setColor(annotation.metadata?.color || "#fbbf24");
      setTags(annotation.metadata?.tags || []);
      setIsMarkdownPreview(true);
    }
  }, [annotation?.id]); // Only reset form when annotation ID changes

  // Update handler with optimistic mutation
