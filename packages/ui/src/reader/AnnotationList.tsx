...
import { usePDFAnnotations } from "@deeprecall/data/hooks";
import { useAnnotationUI, useReaderUI } from "@deeprecall/data/stores";
...
export function AnnotationList({
  ...
  // Use Electric hook for merged annotations (synced + local optimistic changes)
  const { data: annotations, isLoading } = usePDFAnnotations(sha256 || "");
  ...
  // Group by page
  const annotationsByPage = sortedAnnotations.reduce(
    (acc, ann) => {
