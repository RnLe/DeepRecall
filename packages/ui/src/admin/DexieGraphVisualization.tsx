 ...
 *
 * Platform-agnostic graph visualization of database structure using d3-force layout.
...
import { WorkCardCompact } from "../library/WorkCardCompact";
...
export interface DexieGraphVisualizationOperations {
  /** Platform-specific navigation to reader with PDF */
  navigateToReader: (sha256: string, title: string) => void;
  /** Get WorkCard operations for hover tooltip */
  getWorkCardOperations: () => any;
}

export interface DexieGraphVisualizationProps {
  /** Extended works data from platform-specific hook */
  works: WorkExtended[] | undefined;
  operations: DexieGraphVisualizationOperations;
}

export function DexieGraphVisualization({
  works,
  operations,
}: DexieGraphVisualizationProps) {
    ...
    const simulation = d3Force
      ...
      .force(
        ...
        d3Force
          ...
          .radius(
            (d: GraphNode) => NODE_SIZES[d.type as keyof typeof NODE_SIZES] + 10
          )
      )
      .alphaDecay(0.05)
      .velocityDecay(0.6);
          ...
          >
            {hoveredNode.type === "work" ? (
              <div className="scale-75 origin-top-left" style={{ width: 400 }}>
                <WorkCardCompact
                  work={hoveredNode.data as WorkExtended}
                  operations={operations.getWorkCardOperations()}
                />
              </div>
            ) : (
