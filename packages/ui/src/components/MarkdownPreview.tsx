 ...
 */
export interface MarkdownPreviewOperations {
  /** Save content and get new hash */
  saveContent: (params: {
    sha256: string;
    content: string;
    filename: string;
...
export interface MarkdownPreviewProps {
  ...
  /** Platform-specific operations (required if sha256 provided) */
...
export function MarkdownPreview({
  ...
  // Enable editing if sha256 and operations are provided
  const isEditable = !!sha256 && !!operations;
  ...
  // Save content using platform-specific operation
  const handleSave = useCallback(async () => {
    if (!sha256 || !hasUnsavedChanges || !operations) return;

    try {
      setIsSaving(true);
      setSaveStatus("idle");

      const result = await operations.saveContent({
        sha256,
        content,
        filename,
      });
  ...
  }, [sha256, hasUnsavedChanges, content, filename, operations, onSaved]);
  ...
  const handleFilenameBlur = async () => {
    ...
    // Save filename change if sha256 and operations are provided
    if (sha256 && operations && finalFilename !== title) {
      try {
        await operations.renameFile({
          sha256,
