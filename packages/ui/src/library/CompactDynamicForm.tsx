/**
 * CompactDynamicForm Component
 * A more compact version of DynamicForm for editing existing entities
 * Uses smaller spacing, fonts, and more efficient layout
 */
...
interface CompactDynamicFormProps {
...
export function CompactDynamicForm({
  preset,
  initialValues = {},
  onSubmit,
  onCancel,
  ...
  // Get icon for group (smaller versions)
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {Array.from(fieldGroups.entries()).map(([groupName, fields]) => {
        ...
        return (
          <div
            key={groupName}
            className="bg-neutral-800/30 border border-neutral-700 rounded-lg p-3"
          >
            ...
            <div className="space-y-3">
              ...
              {/* Regular fields in grid - more columns */}
        ...
        <div className="flex gap-2">
          {onCancel && (
            <button
              ...
              className="px-4 py-2 text-sm bg-neutral-800 text-neutral-200 rounded-lg hover:bg-neutral-750 transition-colors disabled:opacity-50 disabled:cursor-not-allowed font-medium"
            >
              {cancelLabel}
            </button>
          )}
          <button
            type="submit"
            disabled={isSubmitting}
            className="px-4 py-2 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed font-medium flex items-center gap-2"
          >
            {isSubmitting ? (
              <>
                <svg
...
/**
 * Compact renderer for core schema fields
 */
interface CompactCoreFieldRendererProps {
  fieldKey: string;
  label: string;
  required: boolean;
  value: unknown;
...
function CompactCoreFieldRenderer({
  fieldKey,
  label,
  required,
  value,
  ...
  placeholder,
  helpText,
}: CompactCoreFieldRendererProps) {
  const fieldId = `compact-core-field-${fieldKey}`;
  const inputClass = `w-full px-2.5 py-1.5 text-sm bg-neutral-800 border rounded-lg text-neutral-100 placeholder-neutral-500 focus:outline-none focus:ring-2 transition-colors ${
    error
  ...
  const isUrl =
    fieldKey === "url" || fieldKey === "doi" || fieldKey === "arxivId";
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <label
        htmlFor={fieldId}
        className="text-xs font-medium text-neutral-300 flex items-center gap-1"
      >
        {label}
