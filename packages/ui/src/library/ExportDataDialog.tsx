/**
 * ExportDataDialog Component (Platform-agnostic)
...
export function ExportDataDialog({
  ...
  useEffect(() => {
    ...
    isOpen,
    options.includeDexie,
    options.includeSQLite,
  ...
  const handleExport = async () => {
    ...
    try {
      await exportOps.exportData(options);
      onClose();
    } catch (err) {
              ...
              <label htmlFor="includeDexie" className="flex-1">
                ...
                {estimatedSize && (
                  <p className="text-xs text-zinc-500 mt-1">
                    ~{exportOps.formatBytes(estimatedSize.dexie)}
                  </p>
                )}
              </label>
            </div>
            ...
            <div className="flex items-start gap-3 p-4 bg-zinc-50 dark:bg-zinc-800/50 rounded-lg">
              ...
              <label htmlFor="includeSQLite" className="flex-1 cursor-pointer">
                ...
                {estimatedSize && options.includeSQLite && (
                  <p className="text-xs text-zinc-500 mt-1">
                    ~{exportOps.formatBytes(estimatedSize.sqlite)}
                  </p>
                )}
              </label>
            </div>
            ...
            <div className="flex items-start gap-3 p-4 bg-zinc-50 dark:bg-zinc-800/50 rounded-lg">
              ...
              <label htmlFor="includeFiles" className="flex-1 cursor-pointer">
                ...
                {estimatedSize && options.includeFiles && (
                  <p className="text-xs text-zinc-500 mt-1">
                    ~{exportOps.formatBytes(estimatedSize.files)}
                  </p>
                )}
              </label>
            </div>
          ...
          {estimatedSize && (
            <div className="p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
              <p className="text-sm font-medium text-blue-900 dark:text-blue-100">
                ...
                {exportOps.formatBytes(estimatedSize.total)}
              </p>
